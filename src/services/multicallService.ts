...
class MulticallService {
  ...
  private async initialize(networkKey?: string): Promise<void> {
    try {
      // Get provider for the specified network (or default)
      this.provider = await providerService.getReadyProvider(networkKey);
      ...
      // console.log('[MulticallService] Initialized successfully on network:', network.chainId);
    } catch (error) {
      console.error('[MulticallService] Failed to initialize:', error);
      console.error('[MulticallService] Make sure the RPC endpoint is accessible');
      this.initPromise = null; // Reset so it can retry
      throw new Error('Failed to connect to RPC endpoint. Please check your network connection or try a different network.');
    }
  }

  /**
   * Clear the multicall service state (useful when switching networks)
   */
  clear(): void {
    this.provider = null;
    ...
    // console.log('[MulticallService] Cleared');
  }

  /**
   * Reinitialize with a specific network
   */
  async reinitialize(networkKey: string): Promise<void> {
    // console.log('[MulticallService] Reinitializing for network:', networkKey);
    this.clear();
    await this.initialize(networkKey);
  }
  ...
  async getAllBalances(
    ...
    // console.log('[MulticallService] getAllBalances called:', { userAddress, tokenAddresses });
    await this.ensureInitialized();

    // Get ETH balance separately (can't be multicalled)
    // console.log('[MulticallService] Fetching ETH balance...');
    const ethBalancePromise = this.provider!.getBalance(userAddress);

    // Get all token balances in one multicall
    // console.log('[MulticallService] Fetching token balances...');
    ...
    // console.log('[MulticallService] Balances fetched:', {
      // eth: eth.toString(),
      // tokens: Object.entries(tokens).reduce((acc, [addr, bal]) => ({ ...acc, [addr]: bal.toString() }), {})
    // });
  ...
  async tryAggregate(calls: Call[]): Promise<CallResult[]> {
    try {
      ...
      if (error.code === 'NETWORK_ERROR' || error.event === 'noNetwork') {
        throw new Error('Network connection failed. Please check if your RPC endpoint is accessible.');
      }
