...
const TOKENS_API_URL = import.meta.env.VITE_TOKENS_API_URL || (isDevelopment ? 'http://localhost:8090/api' : 'https://trollbox.noma.money/api');
...
export const tokenApi = {
  ...
  // Get all tokens
  async getTokens(options?: { includeAll?: boolean; chainId?: number }): Promise<{ tokens: TokenResponse[] }> {
    try {
      ...
      const url = `${TOKENS_API_URL}/tokens${queryString ? '?' + queryString : ''}`;
      // console.log('[tokenApi] Fetching tokens from:', url, 'chainId:', options?.chainId);
      const response = await fetch(url);

      if (!response.ok) {
        console.error('[tokenApi] HTTP error:', response.status, response.statusText);
        throw new Error(`HTTP error! status: ${response.status}`);
      }

      const data = await response.json();
      // console.log('[tokenApi] Received tokens:', data.tokens?.length || 0, 'tokens for chainId:', options?.chainId);
      return data;
    } catch (error) {
