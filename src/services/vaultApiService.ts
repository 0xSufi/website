...
export class VaultApiService {
  ...
  constructor(baseUrl?: string) {
    this.baseUrl = baseUrl || config.features.vault.apiUrl;
  }
  ...
  async fetchVaults(address?: string): Promise<VaultInfo[]> {
    ...
    try {
      const url = address
        ? `${this.baseUrl}/vaults?address=${address}`
        : `${this.baseUrl}/vaults`;

      console.log('[VaultApiService] Fetching vaults from:', url);
      ...
      console.log('[VaultApiService] Received vaults:', vaults);
