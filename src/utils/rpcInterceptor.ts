...
// Get chain ID from URL by matching against configured RPC URLs
function getChainIdFromUrl(url: string): { hex: string; dec: string } {
  // Check if it's localhost
  if (url.includes('localhost') || url.includes('127.0.0.1')) {
    return { hex: '0x539', dec: '1337' }; // localhost chainId
  }

  // Match URL against configured networks
  for (const [key, network] of Object.entries(SUPPORTED_NETWORKS)) {
    if (url.includes(network.rpcUrl.replace(/^https?:\/\//, ''))) {
      const chainId = network.chainId;
      return {
        hex: '0x' + chainId.toString(16),
        dec: chainId.toString()
      };
    }
  }

  // Default to monadTestnet if we can't determine
  return { hex: '0x279f', dec: '10143' };
}
...
export function createInterceptedFetch() {
  ...
  window.fetch = async function(...args) {
    ...
    // Don't intercept localhost requests at all
    const isLocalhost = typeof url === 'string' && (url.includes('localhost') || url.includes('127.0.0.1'));
    if (isLocalhost) {
    ...
    if (isRPC) {
      ...
      if (body) {
        ...
        // Block excessive calls for certain methods (but not for localhost)
        const isLocalhost = typeof url === 'string' && (url.includes('localhost') || url.includes('127.0.0.1'));
        ...
        // Don't block requests to localhost - let them all through
        if (threshold && requestCounts[body.method] > threshold && !isLocalhost) {
          // Silently block excessive calls

          // Determine chain ID based on URL
          const chainIds = getChainIdFromUrl(typeof url === 'string' ? url : '');

          // Return cached or default responses
          const defaultResponses: Record<string, any> = {
            'eth_accounts': [],
            'eth_chainId': chainIds.hex,
            'net_version': chainIds.dec,
            'eth_blockNumber': '0x1',
    ...
    if (isRPC) {
      try {
        ...
        console.warn('[RPC Interceptor] Failed to cache response:', error);
      }
    }
