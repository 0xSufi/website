...
// Format very small numbers by showing significant figures
export function formatSmallNumber(value, sigFigs = 4) {
  if (value == null || value === "") return "0";

  const num = Number(value);
  if (isNaN(num) || num === 0) return "0";

  // For numbers >= 0.01, use regular formatting
  ...
  const decimals = Math.max(0, -log + sigFigs - 1);

  // Cap at reasonable number of decimals
  const cappedDecimals = Math.min(decimals, 18);

  return num.toFixed(cappedDecimals);
}
...
export const commifyDecimals = (number, decimals = 4) => {
  ...
  // Handle extreme values that would produce scientific notation
  if (!isFinite(number)) return "0.00";

  // Convert to number and check if it's too large
  const num = Number(number);
  if (Math.abs(num) > 1e15) {
    // Value too large, return scientific notation or capped value
  ...
  let [integerPart, decimalPart] = numStr.split(".");
  integerPart = integerPart.replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  // Don't convert back to Number - it can't parse strings with commas!
  return decimalPart ? `${integerPart}.${decimalPart}` : integerPart;
};
...
/**
 * Formats a token symbol for display by converting it to uppercase
 * @param symbol The token symbol to format (e.g., "gna", "mon", "wmon")
 * @returns Uppercase version of the symbol (e.g., "GNA", "MON", "WMON")
 */
...
/**
 * Formats large token balances compactly with K/M/B suffixes
 * Handles overflow cases where numbers don't fit in the UI
 * @param value The number to format
 * @param decimals Number of decimal places (default 2)
 * @returns Formatted string (e.g., "1.23B", "456.7M", "12.3K")
 */
export function formatBalanceCompact(value, decimals = 2) {
  if (value === undefined || value === null || isNaN(value)) return '0';

  const num = Number(value);
  if (num === 0) return '0';

  // Check for extreme values that would cause scientific notation
  if (!isFinite(num)) return 'Invalid';

  const absValue = Math.abs(num);
  const sign = num < 0 ? '-' : '';

  // For extremely large values (> 1 quadrillion), cap the display
  if (absValue >= 1e15) {
    return sign + '>999T';
  }
  ...
  if (absValue >= 1_000_000_000_000) {
    const val = absValue / 1_000_000_000_000;
    // Avoid toFixed for very large numbers
    const formatted = val < 1000 ? val.toFixed(decimals) : Math.floor(val).toString();
    return sign + formatted + 'T';
  }
  // Billions
  if (absValue >= 1_000_000_000) {
    const val = absValue / 1_000_000_000;
    return sign + val.toFixed(decimals) + 'B';
  }
  // Millions
  ...
  if (absValue >= 10_000) {
    ...
    return sign + val.toFixed(decimals) + 'K';
  }

  // For smaller values, use regular commify
  return commify(absValue, decimals);
}
