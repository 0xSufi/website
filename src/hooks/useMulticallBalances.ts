...
export function useMulticallBalances({
  ...
  // Get selected network to trigger refetch on network change
  ...
  // Include selectedNetwork in the dependency array to refetch when network changes
  const fetchBalances = useCallback(async () => {
    if (!userAddress || !enabled) return;

    // Include network in cache key to ensure different networks have separate caches
    const cacheKey = `balances-${selectedNetwork}-${userAddress}-${tokenAddress || 'none'}`;

    try {
      setIsLoading(true);
      setError(null);

      // console.log('[useMulticallBalances] Fetching balances for network:', selectedNetwork);
  ...
  }, [userAddress, wethAddress, tokenAddress, enabled, selectedNetwork]);
